...
public:
    ...
    bool OnUse(Player* player, Item* item, const SpellCastTargets&)
    {
        ...
        {
            player->GetSession()->SendNotification(GetText(player, "", "Coins disabled."));
        ...
        else
        {
            if (player->HasItemCount(item->GetEntry(), 1, true))
            {
                ...
                coins_amount_message_ru << "" << Coins / 10000;
                coins_amount_message_en << "Merci de votre contribution, vous venez de recevoir des pièces de donation : " << Coins / 10000;
